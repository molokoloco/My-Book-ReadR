...
var readr = function() { // Your welcome to the events-o-drome !
	...
	var disqus_thread		= '#disqus_thread', // ID element + HASH url, imposed by Disqus
		...
		$formNotes			= $notes.find('#formNotes'),
		overlay				= '#overlay',
		$overlay			= $(); 
	...
	var buildEditable = function() {
			...
			$('#formEdit').prepend('<button type="submit" id="saveNotes" title="Téléchargez vos notes dans un fichier .txt" accesskey="s 0">Sauver</button>');
			$textEdit.tinymce({
	...
	var event2key = {'37':'left', '39':'right', '38':'up', '40':'down', '13':'enter', '17':'ctrl', '27':'esc', '32':'space', '107':'+', '109':'-', '33':'pageUp', '34':'pageDown',
					...
					'48':'0', '49':1, '50':2, '51':3, '52':4, '53':5, '54':6, '55':7, '56':8, '57':9}, // à&é"'(-è_ç // Azerty
		...
		pageUnKey = function(event) { // Validate key
			// keyPressed = false;
			if ('activeElement' in document && $(document.activeElement).attr('editable') == true) return; // Don't intercept content edit
			var key = event2key[(event.which || event.keyCode)] || '',
				action = false;
	...
	var showHelp = function(event) { // Show Help div (#helpPanel) // You call a pop, overlay close it...
			if (_db_) db('showHelp()');
			event.preventDefault();
			popCallBack = 'hideHelp'; // Overlay click call this
			showOverlay();
			var middleTop = (getWinHeight() - $helpPanel.outerHeight()) / 2;
			$helpPanel
				.css({opacity:0}).show()
		...
		showOverlay = function(timeout) {
			...
			if (!$overlay.lenght) { // Create overlay
				$overlay = $('<div id="overlay" title="Fermer"/>').appendTo($body); // Create
	...
	var counter = 5;
		blinkHelp = function() {
			...
			($help.is(':visible') ? $help.fadeOut(300) : $help.fadeIn(300));
			counter--;
